...
| [.cursor/skills/client-site-netlify/SKILL.md](.cursor/skills/client-site-netlify/SKILL.md) | Client sites, Netlify, `?preview=`, ask-before-send + 3d→7d×4 follow-ups (discount stack to 50% + commission; FU5 last check-in) |
...
7. Add host to `src/v3/config/previewWhitelist.js` (optional **`previewSettings.fields`** for parent knobs); update `client.json` (`quotation.previewUrl` uses `?preview={slug}`)
...
**Flexible preview settings (parent-owned):** knobs on `carlmanuel.com/?preview=` from `previewWhitelist` schema defaults each session (no held/restored website settings). Parent APPLY → site iframes only; **Save** = `POST /previewSettings` notify (email+push). Site needs markup hooks + apply-only `preview-settings-bridge.js`; admin is **not** the knob surface. `_template` + **XKR** ship this. **Batch retrofit** existing demos (oldest / follow-up clients): hooks + whitelist fields + Netlify redeploy — see client-site-netlify skill; do not auto-resend outreach; images via `POST /previewSettings/upload`.

**Batch upgrade (existing clients):** oldest first; wire `/admin/`, redeploy, QA 4 iframes — do **not** auto-resend initials. Optional second pass: add apply-only bridge + whitelist fields (template already has the script).
